...
class OrderDetailBloc extends Bloc<OrderDetailBaseEvent, OrderDetailBaseState> {
  ...
  void mapEventToState(
    OrderDetailBaseEvent event,
    Emitter<OrderDetailBaseState> emit,
  ) async {
    if (event is OrderDetailFetchDataEvent) {
      try {
        OrderDetail orderDetailModel = await repository!.getOrderDetails(
          event.orderId ?? 1,
        );
      ...
      try {
        AddToCartModel? baseModel = await repository?.reOrderCustomerOrder(
          event.id,
        );
