...
import 'package:bagisto/screens/sign_in/utils/index.dart';
...
class SignInBloc extends Bloc<SignInBaseEvent, SignInBaseState> {
  ...
  void mapEventToState(
    SignInBaseEvent event,
    Emitter<SignInBaseState> emit,
  ) async {
    if (event is FetchSignInEvent) {
      try {
        SignInModel signInModel = await repository!.callSignInApi(
          event.email ?? "",
          event.password ?? "",
        );
        if (signInModel.success == true) {
          emit(
            FetchSignInState.success(
