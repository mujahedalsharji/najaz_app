...
class _BundleOptionsViewState extends State<BundleOptionsView> {
    ...
    widget.options?.forEach((element) {
      element.bundleOptionProducts?.forEach((item) {
        if (item.isDefault == true) {
          ...
          (bundleData['bundleOptionQuantity'] as Map<String, dynamic>?)?[element
                  .id
                  .toString()] =
              item.qty.toString();
          bundleData["bundleOptions"]?[element.id.toString()] = [
            int.parse(item.id ?? "0"),
          ];
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                StringConstants.totalAmount.localized(),
                ...
                setState(() {
                  if (isChecked) {
                    ...
                    _updateOptions(
                      ...
                      qty:
                          (bundleData['bundleOptionQuantity']
                                  as Map<String, dynamic>?)?[option?.id
                                  .toString()]
                              ?.toString() ??
                          '',
                    );
          ...
          Container(
            child: CommonWidgets().getDropdown(
              ...
              (label, key) {
                ...
                selectedProductName =
                    bundleOption?.product?.name.toString() ??
                    "$defaultProductName ";
  ...
  _updateOptions(
    ...
    if (bundleData['bundleOptions'] != null) {
      ...
      if (bundleData['bundleOptions'][id.toString()] != null) {
        if (removeId != -1) {}
        (bundleData['bundleOptions']?[id.toString()] as List<dynamic>).remove(
          removeId,
        );
        ...
        if (productId != -1) {
          if (isReplace) {
            ...
            (bundleData['bundleOptions'][id.toString()] as List<dynamic>).add(
              productId,
            );
          }
        }
  ...
  _updateQtyValue(int id, dynamic qty) {
    ...
    if (bundleData['bundleOptionQuantity'] != null) {
      (bundleData['bundleOptionQuantity'] as Map<String, dynamic>?)?[id
              .toString()] =
          qty;
    } else {
  ...
  List<Widget> getWidgets({Map? selectedProductQty}) {
    ...
    widget.options?.forEach((e) {
      ...
      BundleOptions? val = widget.options?.firstWhereOrNull(
        (option) => option.id == e.id,
      );
      List<int>? list = bundleData["bundleOptions"]?[e.id.toString()];

      list?.forEach((element) {
        BundleOptionProducts? temp = val?.bundleOptionProducts
        ...
        if (e.type == StringConstants.checkBoxText ||
          ...
          qty =
              selectedOptionsGlobalList[e.id ?? "0"]?.firstWhereOrNull(
                (element) => element[productIdKey] == temp?.productId,
              )?[quantityKey];
        } else {
