...
class _CustomOptionsViewState extends State<CustomOptionsView> {
                  ...
                  for (
                    ...
                    if (optionArray[optionArrayKey]["attributeId"].toString() ==
                      ...
                      if (optionArray[optionArrayKey]["attributeOptionId"]
                              .toString() !=
                          item.toString()) {
                        optionArray[optionArrayKey]["attributeOptionId"] = item;
                        firstAdd = false;
                        break;
                      } else {
                        firstAdd = false;
  ...
  _getOptions(int index) {
    if ((widget.customOptions?.length ?? 0) > index) {
      if (index == 0) {
        var codeList =
            widget.customOptions?[0].options
                ?.map((element) => element.id)
                .toList();
        var options = widget.customOptions?[0].options?.where(
          (element) => codeList?.contains(element.id) ?? false,
        );
        return options?.toList() ?? [];
      } else {
        var codeList =
            widget.customOptions?[index].options?.map((e) => e.id).toList();
        var options = widget.customOptions?[index].options?.where(
          (element) => codeList?.contains(element.id) ?? false,
        );
        return options?.toList() ?? [];
      }
