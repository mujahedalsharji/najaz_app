...
import 'package:bagisto/screens/product_screen/view/widget/customizable_option_view.dart';
import 'package:bagisto/utils/extension.dart';
...
class _ProductTypeViewState extends State<ProductTypeView> {
  ...
  Widget build(BuildContext context) {
    double basePrice =
        double.tryParse(widget.productData?.price ?? '') ??
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (widget.productData?.type == StringConstants.configurable)
                    PriceWidgetHtml(
                      priceHtml: formatCurrency(
                        basePrice,
                        GlobalData.currencyCode,
                      ),
                    )
                  else
                    PriceWidgetHtml(
                      priceHtml: widget.productData?.priceHtml?.priceHtml ?? '',
                    ),
                  CommonWidgets().getHeightSpace(8),
                  HtmlWidget(widget.productData?.shortDescription ?? ""),
            ...
            child: CustomizableOptionView(
              ...
              onOptionSelected: (selectedOption) {
                setState(() {
                  ...
                  customOptionsPrice = _calculateCustomOptionsPrice(
                    selectedOption,
                  );
                });
    ...
    for (var entry in selectedOptions.entries) {
      ...
      if (option.type == 'text' || option.type == 'textarea') {
        ...
        for (final id in selectedIds) {
          var priceOption;
          try {
            priceOption = option.customizableOptionPrices?.firstWhere(
              (p) => p.id == id,
            );
          } catch (_) {
            continue;
