...
import 'package:bagisto/screens/product_screen/bloc/product_page_event.dart';
import 'package:bagisto/screens/product_screen/bloc/product_page_repository.dart';
import 'package:bagisto/screens/product_screen/bloc/product_page_state.dart';
...
class ProductScreenBLoc extends Bloc<ProductScreenBaseEvent, ProductBaseState> {
  ...
  void mapEventToState(
    ProductScreenBaseEvent event,
    Emitter<ProductBaseState> emit,
  ) async {
    if (event is FetchProductEvent) {
      ...
      try {
        ...
        if (cartModel?.success == true) {
          emit(
            AddToCartProductState.success(
              response: cartModel,
              successMsg: cartModel?.message,
            ),
          );
        } else {
          emit(
            AddToCartProductState.fail(
      ...
      try {
        BaseModel? baseModel = await repository?.callAddToCompareListApi(
          event.productId ?? "",
        );
        if (baseModel?.status == true) {
          emit(
            AddToCompareListState.success(
              baseModel: baseModel,
      ...
      try {
        AddWishListModel? addWishListModel = await repository!
            .callWishListDeleteItem(event.productId ?? "");
        if (addWishListModel?.success == true) {
          ...
          emit(
            AddToWishListProductState.fail(
      ...
      try {
        AddToCartModel? removeFromWishlist = await repository
      ...
      emit(
        OnClickProductLoaderState(isReqToShowLoader: event.isReqToShowLoader),
      );
    } else if (event is DownloadProductSampleEvent) {
      try {
        DownloadSampleModel? baseModel = await repository?.downloadSample(
          event.type ?? "",
          event.id ?? "",
        );
        if (baseModel?.success == true) {
          emit(
            DownloadProductSampleState.success(
              model: baseModel,
        ...
        emit(
          DownloadProductSampleState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
      ...
      try {
        BookingSlotsData? slotList = await repository?.getSlots(
          event.bookingId,
          event.selectedDate,
        );
        emit(
          GetSlotState.success(
            slotModel: slotList,
            successMsg: "Slots fetched successfully",
          ),
        );
      } catch (e) {
