...
import '../bloc/checkout_shipping_event.dart';
import '../bloc/checkout_shipping_state.dart';
  ...
  Widget buildUI(BuildContext context, CheckOutShippingBaseState state) {
    if (state is CheckOutFetchShippingState) {
      ...
      if (state.status == CheckOutShippingStatus.fail) {
        return ErrorMessage.errorMsg(StringConstants.serverError.localized());
      }
    }
    if (state is CheckOutShippingInitialState) {
      return SkeletonLoader(
        ...
        builder: Container(
          ...
          child: const Card(margin: EdgeInsets.zero, color: Colors.red),
        ),
      );
    }
        ...
        children: [
          ...
          Card(
            ...
            child: RadioButtonGroup(
              ...
              labels:
                  checkOutSaveAddressModel.shippingMethods
                      ?.map(
                        (e) =>
              ...
              onChange: (value, index) {
                if ((checkOutSaveAddressModel.shippingMethods?.length ?? 0) >
                    0) {
                  if (widget.callBack != null) {
                    widget.callBack!(
                      checkOutSaveAddressModel
