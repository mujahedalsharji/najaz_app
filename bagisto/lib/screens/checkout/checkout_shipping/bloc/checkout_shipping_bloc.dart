...
import 'package:bagisto/screens/checkout/data_model/checkout_save_address_model.dart';
...
class CheckOutShippingBloc
    extends Bloc<CheckOutShippingBaseEvent, CheckOutShippingBaseState> {
  CheckOutShippingRepository? repository;

  CheckOutShippingBloc(this.repository)
    : super(CheckOutShippingInitialState()) {
    on<CheckOutShippingBaseEvent>(mapEventToState);
  }

  void mapEventToState(
    CheckOutShippingBaseEvent event,
    Emitter<CheckOutShippingBaseState> emit,
  ) async {
    if (event is CheckOutFetchShippingEvent) {
      try {
        int id = appStoragePref.getCustomerId();
        SaveCheckoutAddresses? checkOutSaveAddressModel = await repository
        ...
        if (checkOutSaveAddressModel?.responseStatus == false) {
          emit(
            CheckOutFetchShippingState.fail(
              error: checkOutSaveAddressModel?.success,
            ),
          );
        } else {
          emit(
            CheckOutFetchShippingState.success(
              checkOutSaveAddressModel: checkOutSaveAddressModel,
            ),
          );
        }
      } catch (e) {
