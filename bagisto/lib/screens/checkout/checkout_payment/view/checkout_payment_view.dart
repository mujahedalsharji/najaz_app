...
import '../bloc/checkout_payment_event.dart';
import '../bloc/checkout_payment_state.dart';

export 'package:bagisto/screens/checkout/data_model/checkout_save_shipping_model.dart';
...
class _CheckoutPaymentViewState extends State<CheckoutPaymentView> {
    ...
    if (state is CheckOutPaymentInitialState) {
      return SkeletonLoader(
        ...
        builder: Container(
          ...
          child: const Card(margin: EdgeInsets.zero, color: Colors.grey),
        ),
      );
    }
  ...
  _paymentMethods(PaymentMethods checkOutShipping) {
    ...
    var paymentMethods = checkOutShipping.paymentMethods?.where(
      (element) => availablePaymentMethods.contains(element.method),
    );
    return Container(
          ...
          Card(
            ...
            child: Container(
              ...
              child: RadioButtonGroup(
                ...
                labels:
                    paymentMethods?.map((e) => e.methodTitle ?? '').toList() ??
                    [],
                onChange: (value, id) {
                  var payment = checkOutShipping.paymentMethods?.firstWhere(
                    (element) => element.methodTitle == value,
                  );
                  if (widget.callBack != null) {
                    widget.callBack!(payment?.method ?? '');
                  }
                },
              ),
            ),
