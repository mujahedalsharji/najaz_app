...
import 'package:bagisto/screens/drawer_sub_categories/utils/index.dart';
...
class DrawerSubCategoriesBloc
  ...
  void mapEventToState(
    DrawerSubCategoriesEvent event,
    Emitter<DrawerSubCategoriesState> emit,
  ) async {
    if (event is FetchDrawerSubCategoryEvent) {
      emit(DrawerSubCategoryInitialState());
      try {
        GetDrawerCategoriesData? data = await repository
            ?.getDrawerSubCategories(filters: event.filters);
        emit(FetchDrawerSubCategoryState.success(getCategoriesData: data));
      } catch (e) {
        emit(
          FetchDrawerSubCategoryState.fail(
    ...
    if (event is FetchCategoryProductsEvent) {
      try {
        NewProductsModel? data = await repository?.callCategoriesData(
          filters: event.filters,
        );
        emit(FetchCategoryProductsState.success(categoriesData: data));
      } catch (e) {
        emit(
          FetchCategoryProductsState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
      ...
      try {
        ...
        if (addWishListModel?.success == true) {
          emit(
            AddWishlistState.success(
              response: addWishListModel,
              message: addWishListModel?.message,
            ),
          );
        } else {
