...
import 'package:bagisto/data_model/add_to_wishlist_model/add_wishlist_model.dart';
import 'package:bagisto/screens/categories_screen/utils/index.dart';
...
class CategoryBloc extends Bloc<CategoryBaseEvent, CategoriesBaseState> {
  ...
  void mapEventToState(
    CategoryBaseEvent event,
    Emitter<CategoriesBaseState> emit,
  ) async {
    if (event is FetchSubCategoryEvent) {
      ...
      try {
        ...
        if (addWishListModel?.success == true) {
          ...
          emit(
            FetchDeleteAddItemCategoryState.fail(
      ...
      try {
        ...
        if (graphQlBaseModel.graphqlErrors != true) {
          emit(
            AddToCartSubCategoriesState.success(
          ...
          emit(
            AddToCartSubCategoriesState.fail(
      ...
      try {
        ...
        if (baseModel.status == true) {
          emit(
            AddToCompareSubCategoryState.success(
              baseModel: baseModel,
          ...
          emit(
            AddToCompareSubCategoryState.fail(error: baseModel.graphqlErrors),
          );
        }
      } catch (e) {
        emit(
          AddToCompareSubCategoryState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
      ...
      emit(
        OnClickSubCategoriesLoaderState(
          isReqToShowLoader: event.isReqToShowLoader,
        ),
      );
    } else if (event is FilterFetchEvent) {
      try {
        GetFilterAttribute? filterModel = await repository?.getFilterProducts(
          event.categorySlug ?? "",
        );
