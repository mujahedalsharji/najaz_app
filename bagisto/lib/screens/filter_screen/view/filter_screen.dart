import 'package:bagisto/screens/filter_screen/utils/index.dart';
  ...
  void fetchFilterData() {
    ...
    for (var attr in widget.data?.filterAttributes ?? []) {
      ...
      if (matchedFilter.isNotEmpty) {
        final values = matchedFilter["value"]
            .toString()
            .replaceAll('"', "")
            .split(',');
      ...
      appBar: AppBar(
        leading: IconButton(
          ...
          onPressed: () {
            ...
            widget.subCategoryBloc?.add(
              FetchSubCategoryEvent(widget.filters, widget.page),
            );
          },
        ),
        title: Row(
          ...
          children: [
            Text(StringConstants.filterBy.localized()),
            GestureDetector(
              onTap: () {
                setState(() {
                  superAttributes = [];
                  ...
                  children: [
                    ListView.separated(
                      ...
                      itemBuilder: (ctx, idx) {
                        if (!(temp.containsKey(
                          getValueFromDynamic(
                            widget.data?.filterAttributes?[idx],
                            "code",
                          ),
                        ))) {
          ...
          ListView.separated(
            ...
            itemBuilder: (ctx, index) {
              ...
              return CheckboxListTile(
                ...
                onChanged: (isChecked) {
                  if (isChecked == true) {
                    int index = widget.filters.indexWhere(
                      (element) => element["key"] == '"$code"',
                    );
                    if (index >= 0) {
                  ...
                  setState(() {
                    ...
                    if (temp[code]?.contains(
                      ...
                      temp[code]?.add('"${getValueFromDynamic(option, "id")}"');
                    }
