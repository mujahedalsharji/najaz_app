...
import 'package:bagisto/screens/search_screen/utils/index.dart';
...
class SearchBloc extends Bloc<SearchBaseEvent, SearchBaseState> {
  ...
  void mapEventToState(
    ...
    if (event is FetchSearchEvent) {
      try {
        NewProductsModel? products = await repository?.callSearchAPi(
          event.filters,
        );
    ...
    if (event is FetchCategoryPageEvent) {
      try {
        GetDrawerCategoriesData? getDrawerCategoriesData = await repository
            ?.getCategoriesList(filters: event.filters);
        if (getDrawerCategoriesData?.responseStatus == true) {
          emit(
            FetchCategoriesPageDataState.success(
              getCategoriesData: getDrawerCategoriesData,
            ),
          );
        } else {
          emit(
            FetchCategoriesPageDataState.fail(
