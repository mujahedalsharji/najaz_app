...
import 'package:bagisto/screens/order_invoices/utils/index.dart';
...
class _InvoiceScreenState extends State<InvoiceScreen> {
  ...
  void initState() {
    ...
    orderInvoiceBloc?.add(
      OrderInvoiceFetchDataEvent(widget.orderDetailsModel?.id ?? 0),
    );
    super.initState();
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    getOrdersRow(
                      ...
                      invoicesList?.viewInvoices?[0].formattedPrice?.subTotal ??
                          "",
                      context,
                    ),
                    getOrdersRow(
      ...
      return Padding(
        ...
        child: Column(
          ...
          children: [
            const SizedBox(),
            Text(
              "${StringConstants.invoice.localized()} #${invoices?.id}",
              style: Theme.of(
                context,
              ...
              itemBuilder: (context, index) {
                return Column(
                  children: [
                    childData(
                      StringConstants.sku,
                      invoices?.items?[index].sku ?? "",
                      context,
                    ),
                    childData(
                      StringConstants.name,
                      invoices?.items?[index].name ?? "",
                      context,
                    ),
                    childData(
                    ...
                    childData(
                      StringConstants.qty,
                      invoices?.items?[index].qty.toString() ?? "",
                      context,
                    ),
                    childData(
                      StringConstants.subTotal,
