...
import 'package:bagisto/screens/gdpr/bloc/gdpr_bloc.dart';
import 'package:bagisto/screens/gdpr/bloc/gdpr_state.dart';
import 'package:bagisto/screens/gdpr/view/widget/create_request.dart';
import 'package:bagisto/screens/product_screen/view/file_download.dart';
          ...
          listener: (BuildContext context, GdprState state) {
            ...
            if (state is CreateGdprRequestState) {
              if (state.status == CreateGdprRequestStatus.success) {
                ScaffoldMessenger.of(
                  context,
                ).showSnackBar(SnackBar(content: Text('Request created')));
                gdprBloc?.add(FetchGdprRequestsList());
              } else {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(state.error ?? 'Failed to create request'),
                  ),
                );
              }
            }
            if (state is RevokeGdprRequestState) {
              if (state.status == GdprStatus.success) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(state.error ?? 'Failed to revoke request'),
                  ),
                );
              }
            }
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            ...
                            child: Text(
                              StringConstants.createRequest.localized(),
                              style: TextStyle(
                      ...
                      TextField(
                        decoration: InputDecoration(
                          ...
                          hintText:
                              StringConstants.searchScreenTitle.localized(),
                          filled: false,
    ...
    return OutlinedButton(
      onPressed: () {
        if (label == 'PDF') {
          ...
          launchUrl(
            Uri.parse("$baseDomain/customer/account/gdpr/html-view"),
