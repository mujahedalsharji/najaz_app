...
class OrderRefundBloc extends Bloc<OrderRefundBaseEvent, OrderRefundBaseState> {
  ...
  void mapEventToState(
    OrderRefundBaseEvent event,
    Emitter<OrderRefundBaseState> emit,
  ) async {
    if (event is OrderRefundFetchDataEvent) {
      try {
        OrderRefundModel refundData = await repository!.getRefundList(
          event.orderId,
        );
        emit(OrderRefundListDataState.success(refundData: refundData));
      } catch (e) {
