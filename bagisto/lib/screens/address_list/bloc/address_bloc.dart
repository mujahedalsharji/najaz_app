...
import 'package:bagisto/screens/address_list/utils/index.dart';
...
class AddressBloc extends Bloc<AddressBaseEvent, AddressBaseState> {
  ...
  void mapEventToState(
    AddressBaseEvent event,
    Emitter<AddressBaseState> emit,
  ) async {
    if (event is FetchAddressEvent) {
      ...
      try {
        ...
        emit(
          RemoveAddressState.success(
            response: baseModel,
            customerDeletedId: event.id,
          ),
        );
      } catch (e) {
        emit(
          RemoveAddressState.fail(
      ...
      try {
        SetDefaultAddress? baseModel = await repository?.setDefaultAddress(
          event.id,
        );
