...
class AccountInfoBloc extends Bloc<AccountInfoBaseEvent, AccountInfoBaseState> {
  ...
  void mapEventToState(
    AccountInfoBaseEvent event,
    Emitter<AccountInfoBaseState> emit,
  ) async {
    if (event is AccountInfoDetailsEvent) {
      try {
        ...
        emit(
          AccountInfoDetailState.success(
      ...
      try {
        ...
        emit(
          AccountInfoUpdateState.success(
            accountUpdate: accountUpdate,
            successMsg: accountUpdate.message,
          ),
        );
      } catch (e) {
      ...
      try {
        BaseModel? baseModel = await repository?.callDeleteAccountApi(
          event.password ?? "",
        );
        if (baseModel?.success == true) {
          emit(
            AccountInfoDeleteState.success(
              baseModel: baseModel,
