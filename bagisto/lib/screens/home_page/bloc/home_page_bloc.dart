...
import 'package:bagisto/screens/home_page/utils/index.dart';
...
class HomePageBloc extends Bloc<HomePageEvent, HomePageBaseState> {
  ...
  void mapEventToState(
    HomePageEvent event,
    Emitter<HomePageBaseState> emit,
  ) async {
    if (event is AddToCartEvent) {
      try {
        ...
        if (graphQlBaseModel?.success == true) {
          ...
          emit(
            AddToCartState.fail(error: graphQlBaseModel?.graphqlErrors ?? ""),
          );
        }
      } catch (e) {
        emit(
      ...
      try {
        AddWishListModel? addWishListModel = await repository
            ?.addItemToWishlist(event.productId);
        if (addWishListModel?.success == true) {
          ...
          emit(
            FetchAddWishlistHomepageState.fail(
      ...
      try {
        ...
        if (baseModel?.success == true) {
          emit(
            AddToCompareHomepageState.success(
              baseModel: baseModel,
        ...
        emit(
          AddToCompareHomepageState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
    ...
    if (event is FetchHomeCustomData) {
      try {
        ...
        emit(
          FetchHomeCustomDataState.success(homepageSliders: homeSlidersData),
        );
      } catch (e) {
        emit(
          FetchHomeCustomDataState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
    ...
    if (event is FetchAllProductsEvent) {
      try {
        NewProductsModel? products = await repository?.getAllProducts(
          filters: event.filters,
        );
        GlobalData.productsStream.sink.add(products);
        // Store the result in the map using the id
        GlobalData.allProducts[event.id] = products;
        emit(FetchAllProductsState.success(allProducts: products));
        GlobalData.cartCountController.sink.add(
          products?.data?.firstOrNull?.cart?.itemsQty ?? 0,
        );
      } catch (e) {
        emit(
          FetchAllProductsState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
    ...
    if (event is CartCountEvent) {
      ...
      try {
        ...
        emit(
          CustomerDetailsState.success(accountInfoDetails: accountInfoDetails),
        );
      } catch (e) {
        emit(
          CustomerDetailsState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
    ...
    if (event is FetchHomePageCategoriesEvent) {
      try {
        GetDrawerCategoriesData? data = await repository?.getHomeCategoriesList(
          filters: event.filters,
        );
        emit(FetchHomeCategoriesState.success(getCategoriesData: data));
      } catch (e) {
        emit(
          FetchHomeCategoriesState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
    ...
    if (event is SubscribeNewsLetterEvent) {
      try {
        BaseModel? baseModel = await repository?.subscribeNewsletter(
          event.email,
        );
        if (baseModel?.success == true) {
