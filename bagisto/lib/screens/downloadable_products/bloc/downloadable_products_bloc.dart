...
class DownloadableProductsBloc
  ...
  void mapEventToState(
    DownloadableProductsBaseEvent event,
    Emitter<DownloadableProductsBaseState> emit,
  ) async {
    if (event is DownloadableProductsCustomerEvent) {
      try {
        DownloadableProductModel? productsList = await repository!
            .getDownloadableProductCustomerData(
              event.page,
              event.limit,
              title: event.title,
              status: event.status,
        ...
        if (productsList.status == true) {
          emit(
            DownloadableProductsCustomerDataState.success(
      ...
      try {
        DownloadLinkDataModel? linkData = await repository
            ?.dataBase64ProductModel(event.id);
        if (linkData?.success == true) {
          emit(
            DownloadBase64ProductState.success(downloadLinkProduct: linkData),
          );
        } else {
        ...
        emit(
          DownloadBase64ProductState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
