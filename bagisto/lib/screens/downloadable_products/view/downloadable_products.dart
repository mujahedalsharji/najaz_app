...
import 'package:bagisto/screens/downloadable_products/utils/index.dart';
...
class _DownLoadableScreenState extends State<DownLoadableScreen> {
  ...
  _setItemScrollListener() {
    if (scrollController.hasClients &&
      ...
      if (hasMoreData()) {
        page += 1;
        downloadableProductsBloc?.add(
          DownloadableProductsCustomerEvent(
            page,
            limit,
          ...
          IconButton(
            onPressed: () {
              showModalBottomSheet(
                context: context,
                builder:
                    (context) => DownloadableOrderFilters(
            ...
            icon: const Icon(Icons.filter_alt, size: AppSizes.spacingLarge * 2),
          ),
        ],
      ),
  ...
  Widget _downloadableProductsItems(DownloadableProductsBaseState state) {
      ...
      return const SingleChildScrollView(
        child: Padding(
          ...
          child: SkeletonLoader(
            items: 6,
            builder: Card(child: SizedBox(height: 125)),
          ),
        ),
      );
    }
    ...
    if (state is DownloadableProductsCustomerDataState) {
      if (page > 1) {
        productsList?.downloadableLinkPurchases?.addAll(
          state.productsList?.downloadableLinkPurchases ?? [],
        );
        ...
        children: [
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              int available =
                  (linkPurchases?.downloadBought ?? 0) -
