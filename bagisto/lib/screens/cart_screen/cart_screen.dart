...
class _CartScreenState extends State<CartScreen> {
      ...
      listener: (BuildContext context, CartScreenBaseState state) {
        if (state is RemoveCartItemState) {
          if (state.status == CartStatus.fail) {
            ...
            GlobalData.cartCountController.sink.add(
              state.removeCartProductModel?.cart?.itemsQty ?? 0,
            );
          }
        }
        ...
        if (state is MoveToCartState) {
          if (state.status == CartStatus.fail) {
            ...
            ShowMessage.successNotification(
              StringConstants.addedToWishlist.localized(),
              context,
            );
          }
        }
        if (state is UpdateCartState) {
          if (state.status == CartStatus.fail) {
            ...
            ShowMessage.successNotification(
              StringConstants.updateCartSuccess.localized(),
              context,
            );
          }
        }
  ...
  Widget buildContainer(BuildContext context, CartScreenBaseState state) {
    ...
    if (state is FetchCartDataState) {
      if (state.status == CartStatus.success) {
        ...
        GlobalData.cartCountController.sink.add(
          state.cartDetailsModel?.itemsQty ?? 0,
        );
  ...
  _cartScreenBody(CartModel? cartDetailsModel) {
    if (cartDetailsModel?.id == null) {
      ...
      return Container(
        child:
            cartDetailsModel?.items?.isEmpty == true
                ? EmptyDataView(
                  assetPath: AssetConstants.emptyCart,
                  message: StringConstants.emptyCartPageLabel,
                  showDescription: true,
