...
import 'package:bagisto/screens/cart_screen/utils/cart_index.dart';
...
class CartScreenBloc extends Bloc<CartScreenBaseEvent, CartScreenBaseState> {
  ...
  void mapEventToState(
    CartScreenBaseEvent event,
    Emitter<CartScreenBaseState> emit,
  ) async {
    if (event is FetchCartDataEvent) {
      ...
      try {
        AddToCartModel? cartDetailsModel = await repository?.updateItemToCart(
          event.item ?? [],
        );
        emit(
          UpdateCartState.success(
      ...
      try {
        AddToCartModel? removeCartProductModel = await repository
            ?.removeFromCart(event.cartItemId!);
        emit(
      ...
      try {
        ...
        emit(
          RemoveAllCartItemState.success(
      ...
      try {
        AddToCartModel? baseModel = await repository?.moveToWishlist(
          event.id ?? 0,
        );
        if (baseModel?.success == true) {
          emit(MoveToCartState.success(response: baseModel, id: event.id));
        } else {
          emit(
            MoveToCartState.fail(
              error:
                  baseModel?.message ??
