...
import 'package:bagisto/screens/compare/utils/index.dart';
...
class CompareScreenBloc
  ...
  void mapEventToState(
    CompareScreenBaseEvent event,
    Emitter<CompareScreenBaseState> emit,
  ) async {
    if (event is CompareScreenFetchEvent) {
      ...
      isLoading = true;
      try {
        CompareProductsData? compareScreenModel = await repository
            ?.callCompareApi(page, limit);
        ...
        emit(
          CompareScreenFetchDataState.success(
            compareScreenModel:
                CompareProductsData(data: allProducts)
        ...
        emit(
          CompareScreenFetchDataState.fail(
      ...
      try {
        ...
        if (baseModel.status == true) {
          emit(
            RemoveFromCompareState.success(
      ...
      try {
        AddToCartModel? graphQlBaseModel = await repository?.callAddToCartAPi(
          int.parse(event.productId ?? ""),
          event.quantity /* event.params*/,
        );
        if (graphQlBaseModel?.success == true) {
          emit(
            AddToCartCompareState.success(
          ...
          emit(
            AddToCartCompareState.fail(error: graphQlBaseModel?.message ?? ""),
          );
        }
      } catch (e) {
        emit(
          AddToCartCompareState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
      ...
      try {
        AddWishListModel? addWishListModel = await repository!
            .callWishListAddDeleteItem(event.productId);
        ...
        emit(
          AddToWishlistCompareState.success(
        ...
        emit(
          AddToWishlistCompareState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
      ...
      emit(
        OnClickCompareLoaderState(isReqToShowLoader: event.isReqToShowLoader),
      );
      ...
      try {
        ...
        if (baseModel.status == true) {
          emit(
            RemoveAllCompareProductState.success(
              baseModel: baseModel,
              successMsg: baseModel.message,
            ),
          );
          ...
          emit(
            RemoveAllCompareProductState.fail(error: baseModel.graphqlErrors),
          );
        }
      } catch (e) {
        emit(
          RemoveAllCompareProductState.fail(
            error: StringConstants.somethingWrong.localized(),
          ),
        );
      }
