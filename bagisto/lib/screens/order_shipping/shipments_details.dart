import 'package:bagisto/screens/order_shipping/utils/index.dart';
...
class _OrderShipmentsScreenState extends State<OrderShipmentsScreen> {
  ...
  void initState() {
    ...
    orderShipmentsBloc?.add(
      OrderShipmentsFetchDataEvent(widget.orderDetailsModel?.id ?? 0),
    );
    super.initState();
  }
      ...
      return Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "${StringConstants.shipment.localized()} #${shipments?.id}",
              style: Theme.of(
                context,
              ).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold),
            ),
              ...
              itemBuilder: (context, index) {
                return Column(
                  children: [
                    childData(
                      StringConstants.sku,
                      shipments?.items?[index].sku ?? "",
                      context,
                    ),
                    childData(
                      StringConstants.name,
                      shipments?.items?[index].name ?? "",
                      context,
                    ),
                    childData(
                      StringConstants.qty,
                      shipments?.items?[index].qty.toString() ?? "",
                      context,
                    ),
                  ],
                );
