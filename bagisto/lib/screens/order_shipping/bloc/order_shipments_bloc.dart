...
class OrderShipmentsBloc
  ...
  void mapEventToState(
    OrderShipmentsBaseEvent event,
    Emitter<OrderShipmentsBaseState> emit,
  ) async {
    if (event is OrderShipmentsFetchDataEvent) {
      try {
        ShipmentModel shipmentData = await repository!.getShipmentsList(
          event.orderId,
        );
        emit(OrderShipmentsListDataState.success(shipmentData: shipmentData));
      } catch (e) {
