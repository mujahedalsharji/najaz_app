...
import 'package:bagisto/screens/forget_password/utils/index.dart';

class ForgetPasswordBloc
    extends Bloc<ForgetPasswordBaseEvent, ForgetPasswordBaseState> {
  ...
  void mapEventToState(
    ForgetPasswordBaseEvent event,
    Emitter<ForgetPasswordBaseState> emit,
  ) async {
    if (event is ForgetPasswordFetchEvent) {
      try {
        BaseModel baseModel = await repository!.callForgetPasswordApi(
          event.email ?? "",
        );

        if (baseModel.success == true) {
          emit(
            ForgetPasswordFetchState.success(
              baseModel: baseModel,
