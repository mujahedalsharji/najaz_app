...
class _AddNewAddressState extends State<AddNewAddress> {
  ...
  final emailController = TextEditingController(
    text: appStoragePref.getCustomerEmail(),
  );
  ...
  _fetchSharedPreferenceData() async {
    ...
    widget.isEdit ?? false
        ...
        : lastNameController.text = customerUserName!
            .split(" ")
            .elementAt(1); // Lorem
  }
        ...
        if (selectedCountry == null) {
          if (widget.isEdit ?? false) {
            ...
            countryController.text =
                countryList
                    ?.firstWhereOrNull(
                      (element) => element.code == widget.addressModel?.country,
                    )
                    ?.name ??
                "";
  ...
  void dropdownUpdate(String item, Key? key) {
    if (key == const Key('country')) {
      ...
      if (selectedCountry != null) {
        var state = selectedCountry?.states?.firstWhereOrNull(
          (element) => element.defaultName == item,
        );
        if (state != null) {
          selectedState = state;
