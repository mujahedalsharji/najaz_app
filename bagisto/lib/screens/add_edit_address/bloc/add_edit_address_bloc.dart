...
import 'package:bagisto/screens/add_edit_address/utils/index.dart';

import '../../../utils/index.dart';
import '../../address_list/data_model/update_address_model.dart';
import 'add_edit_address_event.dart';
...
class AddEditAddressBloc
  ...
  void mapEventToState(
    AddEditAddressBaseEvent event,
    Emitter<AddEditAddressBaseState> emit,
  ) async {
    if (event is FetchEditAddressEvent) {
      try {
        UpdateAddressModel? updateAddressModel = await repository
            ?.callEditAddressApi(
              event.addressId ?? 0,
              event.companyName ?? "",
              event.firstName ?? "",
              ...
              event.email ?? appStoragePref.getCustomerEmail(),
            );
        ...
        emit(
          FetchEditAddressState.fail(error: "e.toString()===>${e.toString()}"),
        );
      }
